...
#define _DEVICE_TABLE_SIZE__           20  // Resource table size
...
#define _DEVICE_TYPE_DRIVER__        0xfd
...
// Get a device function by its name
...
#ifdef __CORE_MAIN_
	for (uint8_t i=0; i < _DEVICE_TABLE_SIZE__; i++) {
		if ((deviceTable.device_type[i] == _DEVICE_TYPE_DRIVER__) && (deviceTable.device_table[i] != nullptr)) {
			Device deviceDriver;
