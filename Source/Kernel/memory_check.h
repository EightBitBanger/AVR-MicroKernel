...
uint32_t allocate_system_memory(void) {
	...
	for (uint8_t address=0; address < kernelBufferSize; address++) {
		allocator.memory_read(address, readByte);
	...
	for (total_system_memory=0x00000; total_system_memory < 0x40000; ) {
		...
		allocator.memory_write(total_system_memory, 0x55);
		allocator.memory_read(total_system_memory, readByte);
		
		// Check if the byte(s) stuck
#ifdef _TEST_RAM_EXTENSIVELY__
		// Run some more memory cycles
		allocator.memory_write(total_system_memory, 0xaa);
		allocator.memory_read(total_system_memory, readByteTest);
		...
		total_system_memory++;
			...
			if (total_system_memory == 0x00000) {
				...
				for (uint8_t address=0; address < kernelBufferSize; address++) allocator.memory_write(address, kernelBuffer[address]);
