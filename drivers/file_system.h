...
struct Partition {
	...
	char& operator[] (uint32_t address) {return cache[address_pointer + address];}
	...
	void format(uint32_t size_bytes) {
		...
		for (uint32_t i=0; i<size_bytes; i+=_CHUNK_SIZE__) 
			memory_bus.write(address_pointer + i, 0x00);
		...
		for (uint8_t i=0; i < (_CHUNK_SIZE__ * 2); i+=_CHUNK_SIZE__) 
			memory_bus.write(address_pointer + i, 0xff);
	...
	uint32_t getSizeBytes(void) {
		...
		for (uint8_t i=1; i<5; i++) {memory_bus.read(address_pointer + i, partitionSz.byte[i-1]);}
	...
	uint32_t getNumberOfFiles(void) {
		...
		for (uint8_t i=5; i<9; i++) memory_bus.read(address_pointer + i, numberOfFiles.byte[i-5]);
	...
	void setSizeBytes(uint32_t byteSz) {
		...
		for (uint8_t i=1; i<5; i++) {memory_bus.write(address_pointer + i, partitionSz.byte[i-1]);}
		return;
	}
	...
	void setNumberOfFiles(uint32_t numberOfFiles) {
		...
		for (uint8_t i=5; i<9; i++) memory_bus.write(address_pointer + i, fileCounter.byte[i-5]);
		return;
	}
	...
	uint8_t file_open(Partition& partition, const char file_name[], uint8_t name_length) {
		...
		for (chunk_offset=0; chunk_offset <= partitionSz; chunk_offset += _CHUNK_SIZE__) {
			...
			for (uint32_t i=0; i < name_length; i++) {
				...
				if (file_name[i] != filename[i]) break;
...
struct FileSystemLibrary {
	...
	FileSystemLibrary() {
		...
		loadLibrary("filesys", 8, (DriverEntryPoint)fileSystemLibraryEntryPoint);
