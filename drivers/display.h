...
struct DisplayDriver {
	...
	void initiate(void) {
		...
		device_bus.waitStateRead  = 8;
		device_bus.waitStateWrite = 8;
	...
	void write(uint32_t address, char byte) {device_bus.write(device_address + address, byte); return;}
	void read(uint32_t address, char& byte) {device_bus.read(device_address + address, byte); return;}
	...
	void writeString(char String[], uint8_t string_size, uint8_t line=0, uint8_t position=0) {
		...
		for (uint8_t i=0; i < stringSz; i++) device_bus.write(address+i, String[i]);
